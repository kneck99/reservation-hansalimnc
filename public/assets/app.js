(function () {
  ...
  btnQuote?.addEventListener('click', async () => {
    try { await quote(); } catch (err) { log(`금액 계산 실패: ${err.message}`); alert(err.message); }
  });
  btnPrepare?.addEventListener('click', async () => {
    try {
      ...
      log(`예약 검증 실패: ${err.message}`); alert(err.message);
    }
  });
  btnPay?.addEventListener('click', async () => {
    try {
      ...
      log(`결제/예약 완료 실패: ${err.message}`); alert(err.message); location.href = '/fail.html';
    }
  });
  btnQuote?.click();
})();
