  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/quote" && request.method === "POST") {
      try {
        ...
        return json({ ok: true, bookingType, totalAmount });
      } catch (error) {
        return json({ ok: false, error: error.message || "금액 계산 실패" }, 400);
      }
    }
    ...
    if (url.pathname === "/api/create-payment" && request.method === "POST") {
      try {
        ...
        const totalAmount = (body.bookingType || "dorm") === "dorm"
        ...
        return json({ ok: false, error: error.message || "예약 검증 실패" }, 400);
      }
    }
    ...
    if (url.pathname === "/api/verify-payment" && request.method === "POST") {
      try {
        ...
        return json({ ok: false, error: error.message || "결제 검증 실패" }, 400);
      }
    }
    ...
    if (url.pathname === "/api/booking-complete" && request.method === "POST") {
      try {
        ...
        return json({ ok: true, appsScript: data });
      } catch (error) {
        return json({ ok: false, error: error.message || "예약 완료 처리 실패" }, 500);
      }
    }
